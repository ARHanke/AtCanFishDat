...
rem This job is run in SQL-Plus via '@d:\assess\extract\marfis\ziff\testrun9697.sql'
ACCEPT Y CHAR PROMPT 'Enter the Year (yyyy) >';
drop table agglog;
create table agglog as
select d.log_code lcode, (substr(d.log_code,1,12) || substr(d.log_code,14,5)) logcode,substr(e.t_species_id_est,1,3) spec,sum(e.t_live_weight/1000) logwt
...
select (substr(slip_code,1,12) || substr(slip_code,14,5)) slipcode,gear_id gear,substr(species_id,1,3) spec,sum(t_live_weight/1000) slipwt
from cl.slip_detail_&&Y
group by (substr(slip_code,1,12) || substr(slip_code,14,5)),gear_id,substr(species_id,1,3);
...
'S' region_code,
cfv cfv_number,
to_number('19' || substr(logcode,7,2)) yland,
to_number(substr(logcode,9,2)) mland,
to_number(substr(logcode,11,2)) dland,
